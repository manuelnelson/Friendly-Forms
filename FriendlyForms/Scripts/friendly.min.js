...
;FormsApp.factory("constantsService",["$resource",function(a){var b={constantResource:a("/api/constants/",{},{get:{method:"GET",params:{format:"json"}}}),constants:{},initializeConstants:function(){b.constants.length===0&&b.constantResource.get({},function(a){b.constants=a})}};return b}])
...
;var FormCompleteCtrl=function(a,b,c,d,e,f,g,h){function i(){d.formCompletes.get({FormName:b.formName,UserId:b.userId},function(c){if(c.IncompleteForms.length===0){a.NoErrors=!0;var f=d.getOutputPaths(b.formName,b.userId);for(var g=0;g<f.length;g++)e.enableMenu(f[g])}else a.IncompleteForms=c.IncompleteForms,a.NoErrors=!1;a.CheckingFormProgress=!1})}a.storageKey=c.path(),a.formName=b.formName.replace(/([A-Z])/g," $1"),a.CheckingFormProgress=!0,a.NoErrors=!0,i(),a.isStarter=b.formName=="PreliminaryInformation",a.submit=function(a){if(a)return;b.formName==="PreliminaryInformation"?e.getMenu().then(function(){c.path("/Domestic/House/user/"+b.userId)}):c.path(d.getOutputPaths(b.formName,b.userId)[0])},h.currentScope=a,g.setTitle("Form Completed")};FormCompleteCtrl.$inject=["$scope","$routeParams","$location","formCompleteService","menuService","genericService","headerService","$rootScope"]
...
;var ScheduleACtrl=["$scope","$routeParams","$rootScope","scheduleAService","menuService","genericService","headerService","$timeout",function(a,b,c,d,e,f,g,h){a.showPrintButton=!1,d.scheduleAs.get({UserId:b.userId},function(b){b.Income.OtherDetails||(b.Income.OtherDetails="There is no reason for having other income."),b.OtherIncome.OtherDetails||(b.Income.OtherDetails="There is no reason for having other income."),a.scheduleA=b,h(function(){var b=$("#main-content").html();b=b.replace(/<form.*>/,""),b=b.replace(/<input.*>/g,""),b=b.replace(/<footer[^>]*?>([\s\S]*)<\/footer>/,""),$(".html").val(b),$(".name").val("ScheduleA"),g.showOutputHeader(),a.showPrintButton=!0},2500)}),a.submit=function(a){},c.currentScope=a,g.hide()}]
...
;var CSACtrl=function(a,b,c,d,e,f,g,h){a.showPrintButton=!1,d.csas.get({UserId:b.userId},function(b){a.csa=b,h(function(){var b=$("#main-content").html();b=b.replace(/<form[^>]*?>([\s\S]*)<\/form>/,""),b=b.replace(/<footer[^>]*?>([\s\S]*)<\/footer>/,""),$(".html").val(b),$(".name").val("ChildSupportAddendum"),g.showOutputHeader(),a.showPrintButton=!0},2500)}),a.submit=function(a){},c.currentScope=a,g.hide()};CSACtrl.$inject=["$scope","$routeParams","$rootScope","csaService","menuService","genericService","headerService","$timeout"]
...
;var PaymentCtrl=["$scope","$routeParams","$location","paymentService","menuService","headerService","userService",function(a,b,c,d,e,f,g){f.setTitle("Payment"),a.submit=function(){if(a.paymentForm.$invalid)return;var d=b.userId;g.getCurrentUserSession().then(function(a){g.roles.save(null,{UserName:a.UserName,Roles:["FirmAdmin","Lawyer"]},function(){c.path("/Administrator/ClientCases/User/"+d)})})}}]
...
;var ClientCasesCtrl=["$scope","$routeParams","$location","clientCasesService","menuService","headerService","$rootScope","clientService","userService",function(a,b,c,d,e,f,g,h,i){a.clients=[],a.userId=b.userId,i.getUserData(b.userId).then(function(b){a.admin=b,i.getLawFirmUsers(b.LawFirmId).then(function(b){a.attorneys=b})}),h.getClients(b.userId).then(function(b){a.clients=b}),a.openClient=function(a){c.path("/Client/"+a.ClientUserId)},a.archiveClient=function(a){},a.openAttorney=function(a){c.path("/Attorney/AttorneyPage/Attorney/"+a.Id)},f.setTitle("Administrator")}]
...
;var AttorneyPageCtrl=["$scope","$routeParams","$location","attorneyPageService","userService","menuService","headerService","clientService",function(a,b,c,d,e,f,g,h){a.clients=[],a.userId=b.userId,h.getClients(b.userId).then(function(b){a.clients=b}),a.openClient=function(a){c.path("/Attorney/Client/"+a.ClientUserId)},a.archiveClient=function(a){},g.setTitle("Attorney Page")}]
...
;var LoginMenuCtrl=["$scope","$routeParams","$location","loginMenuService","constantsService",function(a,b,c,d,e){e.initializeConstants(),a.$watch(function(){return d.authUser},function(){a.user=d.authUser},!0),d.refresh()}]
