...
;var CSACtrl=function(a,b,c,d,e,f,g,h){a.showPrintButton=!1,d.csas.get({UserId:b.userId},function(b){a.csa=b,h(function(){var b=$("#main-content").html();b=b.replace(/<form.*>/,""),b=b.replace(/<input.*>/g,""),b=b.replace(/<footer[^>]*?>([\s\S]*)<\/footer>/,""),$(".html").val(b),$(".name").val("ChildSupportAgreement"),g.showOutputHeader(),a.showPrintButton=!0},2500)}),a.submit=function(a){},c.currentScope=a,g.hide()};CSACtrl.$inject=["$scope","$routeParams","$rootScope","csaService","menuService","genericService","headerService","$timeout"]
...
;var AttorneyPageCtrl=["$scope","$routeParams","$location","attorneyPageService","userService","menuService","headerService","$rootScope",function(a,b,c,d,e,f,g,h){e.users.getList({ids:[1,2,3]}).then(function(a){console.log(a)}),g.title("Attorney Page")}]
