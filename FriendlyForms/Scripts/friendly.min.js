...
;var IntroductionCtrl=["$scope","$routeParams","$location","menuService","genericService","$rootScope",function(a,b,c,d,e,f){a.path=c.path(),a.submit=function(a){d.nextMenu()},f.currentScope=a,e.refreshPage()}]
...
;var DecisionCtrl=function(a,b,c,d,e,f,g){function h(c){var e=angular.copy(a.extraDecisions);d.extraDecisions.get({ChildId:c},function(f){_.each(f.ExtraDecisions,function(a){var b=_.find(e,function(b){return b.Description==a.Description});b&&(b.Id=a.Id,b.ChildId=a.ChildId,d.extraDecisions.update(null,b,function(){}),e=_.reject(e,function(b){return b.Description==a.Description}))}),_.each(e,function(a){a.ChildId=c,d.extraDecisions.save(null,a,function(){})}),d.decisions.get({ChildId:c},function(e){var f=angular.copy(a.decision);f.UserId=b.userId,f.ChildId=c,typeof e.Id!="undefined"?(f.Id=e.Id,d.decisions.update(null,f,function(){a.showMessage=!0})):d.decisions.save(null,f,function(){a.showMessage=!0})})})}a.path=c.path(),a.showErrors=!1,a.showMessage=!1,a.showExtraErrors=!1,d.children.get({UserId:b.userId},function(c){a.children=c.Children,a.childNdx=_.indexOf(_.pluck(a.children,"Id"),parseInt(b.childId)),a.childName=a.children[a.childNdx].Name,a.menuPath="/Parenting/Decision/user/"+b.userId+"/child/"+a.children[0].Id,e.isActive(a.menuPath)||e.setActive(a.menuPath)}),a.getChildDecision=function(b){a.decision=d.decisions.get({ChildId:b},function(){if(typeof a.decision.Id=="undefined"||a.decision.Id==0)a.decision=$.jStorage.get(a.path)}),d.extraDecisions.get({ChildId:b},function(b){b.ExtraDecisions.length===0&&(a.extraDecisions=[]),a.extraDecisions=b.ExtraDecisions})},a.addExtraDecision=function(){if(a.addDecisionForm.$invalid){a.showExtraErrors=!0;return}a.showExtraErrors=!1,a.extraDecision.ChildId=b.childId,d.extraDecisions.save(null,a.extraDecision,function(b){a.extraDecisions.push(b),a.extraDecision.DecisionMaker=-1,a.extraDecision.Description="",a.addDecisionForm.$setPristine()})},a.submit=function(c,g){if(a.decisionForm.$invalid){e.setSubMenuIconClass(a.menuPath,"icon-pencil icon-red");var h=f.getFormInput("#decisionForm");$.jStorage.set(a.path,h),a.showErrors=!0;return}$.jStorage.deleteKey(a.path),a.decision.UserId=b.userId,a.decision.ChildId=b.childId,typeof a.decision.Id=="undefined"||a.decision.Id==0?d.decisions.save(null,a.decision,function(){}):d.decisions.update({Id:a.decision.Id},a.decision,function(){}),e.setSubMenuIconClass(a.menuPath,"icon-ok icon-green");if(a.extraDecisions.length>0){var i=0;_.each(a.extraDecisions,function(b){d.extraDecisions.update(null,b,function(){i++,a.extraDecisions.length===i&&g&&g()})})}else g&&g()},a.previousChild=function(){a.submit(!1,function(){a.childNdx=_.indexOf(_.pluck(a.children,"Id"),parseInt(b.childId));if(a.childNdx===0){e.previousMenu();return}a.childNdx=a.childNdx-1;var d=a.children[a.childNdx].Id;c.path("/Parenting/Decision/user/"+b.userId+"/child/"+d)})},a.nextChild=function(){a.submit(!1,function(){a.childNdx=_.indexOf(_.pluck(a.children,"Id"),parseInt(b.childId));if(a.childNdx===a.children.length-1){e.nextMenu();return}a.childNdx=a.childNdx+1;var d=a.children[a.childNdx].Id;e.setSubMenuIconClass(a.path,"icon-ok icon-green"),c.path("/Parenting/Decision/user/"+b.userId+"/child/"+d)})},a.copyChild=function(c){a.submit(!1,function(){c===0?_.each(a.children,function(a){a.childId!=b.childId&&h(a.Id)}):h(c)})},a.getChildDecision(b.childId),f.refreshPage(function(){g.currentScope=a})};DecisionCtrl.$inject=["$scope","$routeParams","$location","decisionService","menuService","genericService","$rootScope"]
...
;var CSACtrl=function(a,b,c,d,e,f,g,h){a.showPrintButton=!1,d.csas.get({UserId:b.userId},function(b){a.csa=b,h(function(){var b=$("#main-content").html();b=b.replace(/<form.*>/,""),b=b.replace(/<input.*>/g,""),b=b.replace(/<footer[^>]*?>([\s\S]*)<\/footer>/,""),$(".html").val(b),$(".name").val("ChildSupportAddendum"),g.showOutputHeader(),a.showPrintButton=!0},2500)}),a.submit=function(a){},c.currentScope=a,g.hide()};CSACtrl.$inject=["$scope","$routeParams","$rootScope","csaService","menuService","genericService","headerService","$timeout"]
...
;FormsApp.factory("attorneyPageService",["$resource",function(a){var b={attorneyPages:a("/api/attorneyPages",{},{get:{method:"GET",params:{format:"json"}},update:{method:"PUT",params:{format:"json"}}}),attorneyClients:a("/api/attorneyClients",{},{get:{method:"GET",params:{format:"json"}},getList:{method:"GET",isArray:!0,params:{format:"json"}},update:{method:"PUT",params:{format:"json"}}})};return b}])
;var AttorneyPageCtrl=["$scope","$routeParams","$location","attorneyPageService","userService","menuService","headerService","$rootScope",function(a,b,c,d,e,f,g,h){a.clients=[],a.userId=b.userId,d.attorneyClients.getList({UserId:b.userId},function(b){var c=_.pluck(b,"ClientUserId");c&&c.length>0&&e.userAuths.getList({Ids:c},function(c){var d=_.map(b,function(a){var b=_.find(c,function(b){return b.Id===a.ClientUserId});return{UserId:a.UserId,ClientUserId:a.ClientUserId,CaseNumber:a.CaseNumber,Name:b.DisplayName,Date:b.Date}});a.clients=d})}),a.openClient=function(a){c.path("/Client/"+a.ClientUserId)},a.archiveClient=function(a){},g.setTitle("Attorney Page")}]
;FormsApp.factory("createClientService",["$resource",function(a){var b={};return b}])
;var CreateClientCtrl=["$scope","$routeParams","$location","clientService","courtService","headerService","userService",function(a,b,c,d,e,f,g){a.submit=function(){if(a.createClientForm.$invalid)return;a.user.Email.indexOf(!1)&&(a.user.Email=a.user.Email+"@ourlawfirm.com"),g.register.save(null,a.user,function(f){var g={UserId:f.UserId,County:0,CaseNumber:a.court.CaseNumber,AuthorOfPlan:1,PlanType:1},h={UserId:b.userId,ClientUserId:f.UserId};d.clients.save(null,h,function(){e.courts.save(null,g,function(){c.path("/Attorney/Case/"+h.ClientUserId)})})})},f.setTitle("Create A Client")}]
;FormsApp.factory("clientService",["$resource",function(a){var b={clients:a("/api/AttorneyClients/",{},{get:{method:"GET",params:{format:"json"}},update:{method:"PUT",params:{format:"json"}}})};return b}])
;var ClientCtrl=["$scope","$routeParams","$location","clientService","menuService","headerService","$rootScope",function(a,b,c,d,e,f,g){a.path=c.path(),a.client=d.clients.get({UserId:b.userId},function(){}),a.submit=function(c){if(a.clientForm.$invalid)return;a.client.UserId=b.userId,d.clients.update({Id:a.client.Id},a.client,function(){e.setSubMenuIconClass(a.path,"icon-ok icon-green"),c||e.nextMenu()})},g.currentScope=a,f.refreshPage()}]
